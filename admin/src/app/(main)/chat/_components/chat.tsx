...
import { useChat, useConversations, useMessages } from "./use-chat";

export function Chat() {
  const [chat] = useChat();
  const { conversations, isLoading, refetch } = useConversations();
  const { messages, sendMessage } = useMessages(chat.selected);
  const [showContact, setShowContact] = useState(false);
  const [showThread, setShowThread] = useState(false);
  const isLg = useIsLg();
  const isMobile = useIsMobile();

  const activeConversation = conversations.find((c) => c.id === chat.selected) ?? null;
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <ChatProfileDetails conversation={activeConversation} onClose={() => setShowContact(false)} />
          </div>
        </div>
      </div>
