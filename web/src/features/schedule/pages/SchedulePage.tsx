...
import { MapPin, Clock, ChevronDown, ArrowRight, Users, Calendar, ChevronRight } from 'lucide-react';
  ...
  return (
    <div style={{ position: 'relative', flex: 1 }}>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(240,237,230,0.4)', fontFamily: 'system-ui', marginBottom: 4 }}>
            {label}
          </div>
          <div style={{ fontSize: 15, fontWeight: 600, color: '#f0ede6', fontFamily: 'system-ui' }}>
            {value || 'Chọn địa điểm'}
          </div>
        </div>
        <ChevronDown size={14} style={{ color: 'rgba(240,237,230,0.3)', flexShrink: 0, transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{
              ...
              background: '#161a1a', border: '1px solid rgba(255,255,255,0.1)',
            ...
            {options.map((opt) => (
              <button
                ...
                style={{
                  ...
                  padding: '12px 16px', background: opt === value ? 'rgba(212,175,55,0.1)' : 'none',
                  border: 'none', cursor: 'pointer',
                  fontSize: 13, color: opt === value ? '#d4af37' : 'rgba(240,237,230,0.7)',
                  fontFamily: 'system-ui', transition: 'all 0.15s',
                  borderLeft: opt === value ? '2px solid #d4af37' : '2px solid transparent',
                }}
              >
                {opt}
              </button>
...
function ScheduleCard({ item, index }: { item: typeof SCHEDULES[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ borderColor: 'rgba(212,175,55,0.3)', backgroundColor: 'rgba(212,175,55,0.03)' }}
    >
      {/* Time */}
      <div style={{ padding: '22px 0', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2rem', fontWeight: 600, color: '#f0ede6', lineHeight: 1 }}>
          {item.dep}
        </div>
        <div style={{ fontSize: 10, color: 'rgba(240,237,230,0.35)', fontFamily: 'system-ui', marginTop: 4, letterSpacing: '0.05em' }}>
          → {item.arr}
        </div>
      </div>
      ...
      <div style={{ padding: '22px 24px', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: '#f0ede6', fontFamily: 'system-ui' }}>{item.from}</span>
          <ArrowRight size={11} style={{ color: '#d4af37', flexShrink: 0 }} />
          <span style={{ fontSize: 13, fontWeight: 600, color: '#f0ede6', fontFamily: 'system-ui' }}>{item.to}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 10, color: 'rgba(240,237,230,0.35)', fontFamily: 'system-ui' }}>{item.operator}</span>
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(240,237,230,0.25)', fontFamily: 'system-ui', padding: '2px 8px', border: '1px solid rgba(255,255,255,0.08)' }}>
            {item.type}
          </span>
        </div>
      </div>
      ...
      {/* Duration */}
      <div style={{ padding: '22px 20px', borderRight: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 12, color: 'rgba(240,237,230,0.5)', fontFamily: 'system-ui' }}>
          <Clock size={11} style={{ color: '#d4af37' }} /> {item.duration}
        </div>
      </div>
      ...
      <div style={{ padding: '22px 20px', borderRight: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
          <Users size={11} style={{ color: lowSeats ? '#e05c5c' : 'rgba(240,237,230,0.3)' }} />
          <span style={{ fontSize: 12, fontFamily: 'system-ui', color: lowSeats ? '#e05c5c' : 'rgba(240,237,230,0.55)', fontWeight: lowSeats ? 700 : 400 }}>
            {item.seats} chỗ
          </span>
        </div>
        {lowSeats && (
      ...
      <div style={{ padding: '22px 0 22px 20px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10 }}>
        <div>
          <div style={{ fontSize: 17, fontWeight: 700, color: '#d4af37', fontFamily: 'system-ui', textAlign: 'right' }}>
            {item.price.toLocaleString('vi-VN')}₫
          </div>
          <div style={{ fontSize: 10, color: 'rgba(240,237,230,0.3)', fontFamily: 'system-ui', textAlign: 'right', marginTop: 2 }}>
            / người
          </div>
        </div>
        <button style={{
    ...
    <div style={{ background: '#0e1111', color: '#f0ede6', minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', paddingTop: 120, paddingBottom: 0 }}>
        <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', zIndex: 0 }}>
          ...
          />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(14,17,17,0.5) 0%, #0e1111 100%)' }} />
        </div>
        ...
        <div style={{ position: 'relative', zIndex: 10, padding: '0 8% 80px' }}>
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 style={{
              ...
              fontWeight: 400, color: '#f0ede6', lineHeight: 1.1,
              margin: '0 0 12px',
            }}>
              Tìm chuyến xe <em style={{ color: '#d4af37' }}>phù hợp</em>
            </h1>
            <p style={{ fontSize: 14, color: 'rgba(240,237,230,0.45)', fontFamily: 'system-ui', margin: '0 0 48px' }}>
            ...
            <div style={{
              ...
              <div style={{ background: 'rgba(255,255,255,0.07)', width: 1 }} />
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      color: '#f0ede6', fontSize: 14, fontWeight: 600,
            ...
            {POPULAR_ROUTES.map((r, i) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: '#f0ede6', fontFamily: 'system-ui', fontWeight: 600 }}>{r.from}</span>
                  <ArrowRight size={10} style={{ color: '#d4af37' }} />
                  <span style={{ fontSize: 12, color: '#f0ede6', fontFamily: 'system-ui', fontWeight: 600 }}>{r.to}</span>
                </div>
                <div style={{ fontSize: 10, color: 'rgba(240,237,230,0.35)', fontFamily: 'system-ui', display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      {searched && (
        <section style={{ padding: '0 8% 100px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.6rem', fontWeight: 500, color: '#f0ede6' }}>
                  {fromCity}
                </span>
              ...
              <p style={{ fontSize: 11, color: 'rgba(240,237,230,0.35)', fontFamily: 'system-ui', margin: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 10, color: 'rgba(240,237,230,0.3)', fontFamily: 'system-ui', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                Sắp xếp:
              </span>
              {SORT_OPTIONS.map(opt => (
                <button
                  ...
                  style={{
                    background: 'none', border: sortBy === opt ? '1px solid rgba(212,175,55,0.5)' : '1px solid rgba(255,255,255,0.08)',
                    padding: '6px 14px', cursor: 'pointer',
                    fontSize: 10, fontFamily: 'system-ui', fontWeight: 600, letterSpacing: '0.05em',
                    color: sortBy === opt ? '#d4af37' : 'rgba(240,237,230,0.4)',
                    transition: 'all 0.2s',
                  }}
                >
                  {opt}
            ...
            {SLOT_TABS.map(tab => (
              <button
                ...
                style={{
                  ...
                  color: slotFilter === tab ? '#d4af37' : 'rgba(240,237,230,0.3)',
                  borderBottom: slotFilter === tab ? '2px solid #d4af37' : '2px solid transparent',
                  transition: 'all 0.2s',
                }}
              >
                {tab}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {results.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '80px 0', color: 'rgba(240,237,230,0.3)', fontFamily: 'system-ui' }}>
