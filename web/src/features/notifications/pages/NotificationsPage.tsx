...
export function NotificationsPage() {
  ...
  return (
    <div className="bg-[#0e1111] min-h-screen pt-24 pb-20 relative overflow-hidden">
      ...
      <div className="container px-4 max-w-4xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/10 text-gray-400 rounded-full font-bold text-xs mb-4 border border-white/10">
              <Bell className="w-3.5 h-3.5 text-[#d4af37]" /> {t('notifications.center')}
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-3xl md:text-4xl font-black text-white tracking-tight">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            <button 
              ...
              className="flex items-center gap-2 text-sm font-bold text-[#d4af37] hover:text-[#f0c94a] bg-[#d4af37]/10 hover:bg-[#d4af37]/20 px-4 py-2.5 rounded-xl transition-colors"
            >
        ...
        <motion.div 
          ...
          className="flex flex-wrap gap-2 mb-8 bg-white/5 p-2 rounded-2xl border border-white/10 w-fit"
        >
          {FILTERS.map(filter => (
            <button
              key={filter.id}
              ...
              className={`px-5 py-2 rounded-xl font-bold text-sm transition-all duration-300 ${
                activeFilter === filter.id
                  ? 'bg-[#d4af37] text-[#0e1111] shadow-md'
          ...
          <AnimatePresence mode="popLayout">
            {filteredNotifications.length > 0 ? (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-white mb-2">{t('notifications.empty')}</h3>
                <p className="text-gray-400 font-medium">{t('notifications.emptyDesc')}</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
