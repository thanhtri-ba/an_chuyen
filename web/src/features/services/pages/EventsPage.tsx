        ...
        <div style={{ background: '#fcfcfc', color: '#1a1a1a', minHeight: '100vh', paddingTop: 100, paddingBottom: 80, fontFamily: 'system-ui' }}>
            <div style={{ padding: '0 8%' }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ maxWidth: 1200, margin: '0 auto' }}>
                    <ServicePageHeader
                        icon="https://raw.githubusercontent.com/microsoft/fluentui-emoji/main/assets/Party%20popper/3D/party_popper_3d.png"
                        title={t('services.events.title', 'Sự Kiện Nổi Bật')}
                    />

                    {loading ? (
                        <div style={{ display: 'flex', justifyContent: 'center', padding: '80px 0' }}>
                            <Loader2 style={{ width: 32, height: 32, color: '#163328' }} className="animate-spin" />
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {events.map(event => (
                                <div key={event.id} className="group" style={{ background: 'rgba(0,0,0,0.025)', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 16, overflow: 'hidden', transition: 'border-color 0.3s' }}>
                                    <div style={{ height: 192, position: 'relative', overflow: 'hidden' }}>
                                        ...
                                        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(255,255,255,0.9), transparent 50%)' }} />
                                    </div>
                                    <div style={{ padding: 24 }}>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                                            <span style={{ background: 'rgba(22,51,40,0.12)', color: '#163328', padding: '5px 12px', borderRadius: 100, fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                                        ...
                                        <button style={{
                                            ...
                                            padding: '12px 16px', fontWeight: 700, fontSize: 13, border: '1px solid rgba(0,0,0,0.1)',
                                            borderRadius: 8, cursor: 'pointer', transition: 'all 0.2s',
                                        }}
                                            onMouseEnter={(e) => { e.currentTarget.style.background = '#163328'; e.currentTarget.style.color = '#fcfcfc'; }}
