...
exports.buildContext = exports.getUserBookings = exports.getPolicies = exports.searchTrips = exports.extractSearchIntent = void 0;
...
const extractSearchIntent = async (message) => {
    ...
    if (lowerMsg.match(/(từ|đi từ|ở) (sài gòn|hcm|hồ chí minh)/))
        intent.from = 'Hồ Chí Minh';
    else if (lowerMsg.match(/(từ|đi từ|ở) (đà lạt)/))
        intent.from = 'Đà Lạt';
    if (lowerMsg.match(/(đến|đi|lên|về) (sài gòn|hcm|hồ chí minh)/))
        intent.to = 'Hồ Chí Minh';
    else if (lowerMsg.match(/(đến|đi|lên|về) (đà lạt)/))
        intent.to = 'Đà Lạt';
    ...
    if (lowerMsg.includes('sáng'))
        intent.timeRange = ['05:00', '11:59'];
    else if (lowerMsg.includes('trưa'))
        intent.timeRange = ['11:00', '14:00'];
    else if (lowerMsg.includes('chiều'))
        intent.timeRange = ['14:01', '17:59'];
    else if (lowerMsg.includes('tối') || lowerMsg.includes('đêm'))
        intent.timeRange = ['18:00', '23:59'];
    else {
    ...
    if (lowerMsg.includes('giường nằm') || lowerMsg.includes('nằm'))
        intent.seatType = 'SLEEPER';
    else if (lowerMsg.includes('limousine') || lowerMsg.includes('vip'))
        intent.seatType = 'VIP';
    else if (lowerMsg.includes('ghế ngồi') || lowerMsg.includes('ngồi'))
        intent.seatType = 'ECONOMY';
...
const searchTrips = async (intent) => {
    const { from, to, date, timeRange, seatType, targetPrice } = intent;
    // Hard filters: Must match from, to, date if provided
    let whereClause = { departureTime: { gte: new Date() } };
    if (from || to) {
        whereClause.trip = { route: {} };
        if (from)
            whereClause.trip.route.departureCity = { name: { contains: from, mode: 'insensitive' } };
        if (to)
            whereClause.trip.route.arrivalCity = { name: { contains: to, mode: 'insensitive' } };
    }
    if (date) {
    ...
    const scoredTrips = candidateSchedules.map(schedule => {
        ...
        if (seatType) {
            ...
            if (!hasSeatType)
                score -= 30; // High penalty for wrong seat type
        }
...
const buildContext = async (message, userId) => {
    ...
    const intent = await (0, exports.extractSearchIntent)(message);
    if (Object.keys(intent).length > 0) {
        contextParts.push(`USER'S PARSED INTENT:\n${JSON.stringify(intent, null, 2)}`);
        const trips = await (0, exports.searchTrips)(intent);
        if (trips.length > 0) {
    ...
    if (contextParts.length === 0) {
        const defaultTrips = await (0, exports.searchTrips)({});
